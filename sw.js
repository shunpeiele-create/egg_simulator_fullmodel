...
<head>
    ...
    <title>EGG MOUSE SIMULATOR v164</title>
    ...
    <style>
        /* システム設定 */
        :root { --primary-bg: #ffffff; --panel-bg: #ffffff; --border: #e5e5ea; --text-sub: #8e8e93; --alert: #ff3b30; --disabled: #d1d1d6; --overlay-dim: rgba(0,0,0,0.4); }
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; -webkit-user-select: none; -webkit-touch-callout: none; user-select: none; outline: none; font-family: 'Futura PT', Futura, 'Noto Sans JP', sans-serif; }
        
        /* 物理全画面化 & 隙間対策 */
        html { background-color: var(--primary-bg); height: 100vh; overflow: hidden; }
        body { margin: 0; padding: 0; overflow: hidden; position: fixed; inset: 0; width: 100vw; height: 100vh; }
        body::before { content: ""; position: fixed; top: -200px; bottom: -200px; left: 0; right: 0; background-color: var(--primary-bg); z-index: -9999; }
        
        small { font-weight: 400 !important; }
        #global-guard { position: fixed; inset: 0; z-index: 99999; display: none; }
        
        /* UI配置 */
        .manual-saver-trigger { position: fixed; top: env(safe-area-inset-top); left: 0; width: 80px; height: 80px; z-index: 10000; cursor: pointer; }
        .admin-trigger { position: absolute; top: calc(15px + env(safe-area-inset-top)); right: 15px; width: 50px; height: 50px; background: none; border: none; cursor: pointer; font-size: 28px; color: #eee; z-index: 15000; display: flex; justify-content: center; align-items: center; }
        ...
        /* 操作パネル (余白ゼロで下詰め) */
        .control-pane { background: var(--panel-bg); padding: 50px 35px 0; padding-bottom: 0 !important; display: flex; flex-direction: column; position: relative; z-index: 10; height: 100%; overflow: hidden; }
        
        /* ボタンエリア (下35px均等配置) ★修正：z-index追加 */
        #btn-container { 
            margin-top: auto; 
            padding-bottom: 35px; 
            background: var(--panel-bg);
            position: relative; /* 追加 */
            z-index: 100;       /* 追加：これでマウス操作でも確実に反応します */
        }
        ...
        @keyframes snapLaunch { 0% { transform: translate(-50%, -65%) scale(1.15); opacity: 1; } 15% { transform: translate(-50%, -45%) scale(1.0); opacity: 1; } 30% { transform: translate(-50%, -75%) scale(1.2); opacity: 1; } 100% { transform: translate(-50%, -200vh) scale(0.2); opacity: 0; } }
        ...
        /* COMMUNITY / SAVER */
        .community-area-box { flex: 1; position: relative; overflow: hidden; background: #fcfcfc; border-radius: 20px; border: 1px dashed var(--border); cursor: grab; touch-action: none; z-index: 5; margin-bottom: 15px; min-height: 100px; }
        #saver-layer { position: fixed; inset: 0; background: #fff; z-index: 9000; cursor: grab; touch-action: none; opacity: 0; pointer-events: none; transition: opacity 0s; }
        body.saver-active #saver-layer { opacity: 1; pointer-events: auto; transition: opacity 1.2s ease; }
        ...
        /* セーバー復帰ガイド (下35px均等配置) */
        #saver-guide { position: fixed; bottom: 35px; right: 35px; width: 540px; opacity: 0; transition: 0.5s; z-index: 11000; pointer-events: none; cursor: pointer; }
        ...
        #thanks-overlay { position: absolute; inset: 0; background: rgba(255,255,255,0.92); display: none; justify-content: center; align-items: center; z-index: 500; flex-direction: column; opacity: 0; transition: opacity 0.4s ease; }
...
<div class="app-grid" id="main-container">
    ...
    <div class="control-pane ui-part">
        ...
        <div id="btn-container">
            <div id="cover-alert" class="alert-text">Please select body color <small>(本体を選んでください)</small></div>
...
<script>
    // ★修正：タイムアウトを実質無効化（999999999ミリ秒）
    const CONFIG = { storageKey: 'egg_mouse_v131_data', settingsKey: 'egg_mouse_v130_set', inactivityLimit: 999999999, baseCount: 5, coverMax: 11, maxItems: 20 };
    ...
    function selectBase(id){
        ...
        if(!selectedCover || selectedCover === 0) {
    ...
    function resetUIState(){
        ...
        document.getElementById('cover-alert').innerHTML = 'Please select body color <small>(本体を選んでください)</small>';
    }
    ...
    function openAdmin(){ const MASTER_PW = "8056"; const pw = prompt("Password"); if(pw === MASTER_PW) { document.getElementById('admin-overlay').classList.add('show'); } else if(pw !== null) { alert("Incorrect password"); } }
    ...
    function onPointerDown(e){ 
        ...
        let found = null; 
        communityItems.forEach(i=>{ const hr = isSaverActive ? 6.5 : 4.5; if(Math.sqrt((tx-i.x)**2+(ty-i.y)**2) < hr) found=i; }); 
        if(found){ grabbedItem=found; grabbedItem.vx=0; grabbedItem.vy=0; } else { 
            communityItems.forEach(i=>{ 
                const dx=i.x-tx, dy=i.y-ty, d=Math.sqrt(dx*dx+dy*dy), r=isSaverActive?20:15; 
                if(d < r){ i.vx+=(dx/d)*(r-d)*0.6; i.vy+=(dy/d)*(r-d)*0.6; i.dr+=(Math.random()-0.5)*15; } 
            }); 
        } 
    }
    ...
    function updateAnimation(){
        // リミット解除＆回転トルク実装
        const lx=isSaverActive?94:92, ly=isSaverActive?93:89, md=isSaverActive?7:8; 
        for(let i=0; i<communityItems.length; i++){
            for(let j=i+1; j<communityItems.length; j++){
                ...
                if(d < md && d > 0.1){ 
                    ...
                    let f;
                    if (isSaverActive) { if (d < 5.5) f = 0.9; else f = (md - d) * 0.04; } else { if (d < 7.5) f = 1.5; else f = (md - d) * 0.16; }
                    a.vx -= Math.cos(ang) * f; a.vy -= Math.sin(ang) * f; b.vx += Math.cos(ang) * f; b.vy += Math.sin(ang) * f;
                    const impact = Math.sqrt((a.vx - b.vx)**2 + (a.vy - b.vy)**2);
                    const spin = 4.0; 
                    a.dr += (Math.random()-0.5) * impact * spin; b.dr += (Math.random()-0.5) * impact * spin;
                }
            }
        }
        communityItems.forEach(i=>{
            ...
            if(i !== grabbedItem){ 
                let speed = Math.sqrt(i.vx * i.vx + i.vy * i.vy);
                const minSpeed = 0.04; 
                if(speed > 0 && speed < minSpeed) { let ratio = minSpeed / speed; i.vx *= ratio; i.vy *= ratio; }
                i.x += i.vx; i.y += i.vy; i.rot += i.dr;
                i.vx = Math.max(-10, Math.min(10, i.vx)); i.vy = Math.max(-10, Math.min(10, i.vy));
                i.vx*=0.985; i.vy*=0.985; i.dr*=0.996; 
                if(i.x < 0){ i.x=0; i.vx*=-0.8; i.dr+=i.vy*0.8; } else if(i.x > lx){ i.x=lx; i.vx*=-0.8; i.dr-=i.vy*0.8; }
                if(i.y < 0){ i.y=0; i.vy*=-0.8; i.dr-=i.vx*0.8; } else if(i.y > ly){ i.y=ly; i.vy*=-0.8; i.dr+=i.vx*0.8; }
            }
    ...
    function startSaver(){ if(isSaverActive)return; isSaverActive=true; document.body.classList.add('saver-active'); communityItems.forEach(i => { document.getElementById('saver-layer').appendChild(i.el); i.el.style.width = "80px"; i.el.style.height = "104px"; }); document.getElementById('saver-guide').style.opacity = 1; document.getElementById('saver-guide').style.pointerEvents = 'auto'; }
    function stopSaver(e){ if(e) e.stopPropagation(); isSaverActive = false; document.body.classList.remove('saver-active'); communityItems.forEach(i => { document.getElementById('community-area').appendChild(i.el); i.el.style.width = "34px"; i.el.style.height = "44px"; }); document.getElementById('saver-guide').style.opacity = 0; document.getElementById('saver-guide').style.pointerEvents = 'none'; resetInactivityTimer(); }
    ...
    function loadCommunity(it){ if(it){ if(validCoverIdsForRandom.length === 0) { setTimeout(() => loadCommunity(true), 500); return; } for(let i=0; i<5; i++){ if(validCoverIdsForRandom.length) addCommunityItem(Math.floor(Math.random()*5)+1, validCoverIdsForRandom[Math.floor(Math.random()*validCoverIdsForRandom.length)], true); } } }
    function addCommunityItem(b,c,isInitial){ if(communityItems.length >= CONFIG.maxItems) { const oldest = communityItems.shift(); oldest.el.style.opacity = '0'; setTimeout(() => oldest.el.remove(), 500); } const el=document.createElement('div'); el.className='community-item'; el.style.width = "34px"; el.style.height = "44px"; el.innerHTML=`<img src="image/base_${b}.png" style="width:100%;position:absolute;"><img src="image/cover_${c}.png" style="width:100%;position:absolute;opacity:${c===0?0:1}" onerror="this.style.opacity=0">`; const it={ el, x:Math.random()*80+5, y:Math.random()*70+5, rot:Math.random()*360, dx:(Math.random()-0.5)*0.10*0, dy:(Math.random()-0.5)*0.10*0, dr:(Math.random()-0.5)*15, vx: (Math.random()-0.5) * 0.8, vy: (Math.random()-0.5) * 0.8, scale: isInitial ? 1 : 0 }; (isSaverActive ? document.getElementById('saver-layer') : document.getElementById('community-area')).appendChild(el); communityItems.push(it); }
